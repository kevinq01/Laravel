...
$reporte->setTitle("Rendimiento postcosecha");


//Posicion del titulo
$reporte->setCellValue('A1', 'Rendimiento empaque postcosecha');
...
$reporte->setCellValue('C2', 'Rendimiento');
...
if ($selectedOption === "1") {
    $Reporte = $report->moduloEmpaquePostcosechaFecha($id,$desde, $hasta);
} else {
    $Reporte = $report->moduloEmpaquePostcosechaSemana($id,$Week);
}
...
if ($Reporte != null) {
    foreach ($Reporte as $rows) {
        ...
        $reporte->setCellValue('C' . $count, $rows['rendimiento']);
...
header('Content-Disposition: attachment;filename="rendimientoPostcosecha_' . $desde . '.xls"');
