...
$reporte->setTitle("Tiempo producción");


//Posicion del titulo
$reporte->setCellValue('A1', 'Tiempo muerto producción');
...
$reporte->setCellValue('C2', 'Tiempo muerto');
...
if ($selectedOption === "1") {
    $Reporte = $report->tiempoProuduccionFecha($desde, $hasta);
} else {
    $Reporte = $report->tiempoProuduccionSemana($Week);
}

if ($Reporte != null) {
    foreach ($Reporte as $rows) {
        $reporte->setCellValue('A' . $count, $rows['laborTP']);
        $reporte->setCellValue('B' . $count, $rows['causa']);
        $reporte->setCellValue('C' . $count, $rows['tiempo']);
...
header('Content-Disposition: attachment;filename="tiempoProduccion-' . $desde . '.xls"');
