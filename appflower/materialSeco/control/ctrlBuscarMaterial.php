...
if ($accion === "btn-buscar-material" && empty($codigo) != 1) {

    $materialSeco = $MaterialSeco->listarMateriaTable($codigo);
    if ($materialSeco != null) {
        ...
        foreach ($materialSeco as $materialSeco) {
            ...
            $promedio =  $materialSeco['Promedio'];
            echo                            "<div class=''>";
            echo                                "<button class='btn btn-block  p-0 border bg-light rounded-0 shadow-none px-2 text-dark' data-toggle='collapse' data-target='#tarjeta$id_seco' aria-expanded='true' aria-controls='tarjeta$id_seco'>";
            ...
            echo                                "</button>";
            echo                            "</div>";
            echo                            "<div class='collapse border border-top-0 ' id='tarjeta$id_seco' data-parent='#accordionModal'>";
